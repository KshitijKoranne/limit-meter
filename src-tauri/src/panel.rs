...
tauri_panel! {
    ...
    panel_event!(LimitMeterPanelEventHandler {
...
pub fn init(app_handle: &tauri::AppHandle) -> tauri::Result<()> {
    ...
    let panel = window.to_panel::<LimitMeterPanel>()?;
    ...
    let event_handler = LimitMeterPanelEventHandler::new();
