...
fn current_macos_keychain_account_from_user_env(user_env: Option<String>) -> String {
    user_env
        ...
        .unwrap_or_else(|| "limitmeter-user".to_string())
}
...
pub fn inject_host_api<'js>(
    ...
    globals.set("__limitmeter_ctx", probe_ctx)?;
...
fn inject_http<'js>(ctx: &Ctx<'js>, host: &Object<'js>, plugin_id: &str) -> rquickjs::Result<()> {
    ...
    ctx.eval::<(), _>(
        r#"
        (function() {
            // Will be patched after __limitmeter_ctx is set.
            if (typeof __limitmeter_ctx !== "undefined") {
                void 0;
            }
        })();
        "#
...
pub fn patch_http_wrapper(ctx: &rquickjs::Ctx<'_>) -> rquickjs::Result<()> {
    ctx.eval::<(), _>(
        r#"
        (function() {
            var rawFn = __limitmeter_ctx.host.http._requestRaw;
            __limitmeter_ctx.host.http.request = function(req) {
...
/// Inject utility APIs (line builders, formatters, base64, jwt) onto __limitmeter_ctx
pub fn inject_utils(ctx: &rquickjs::Ctx<'_>) -> rquickjs::Result<()> {
    ctx.eval::<(), _>(
        r#"
        (function() {
            var ctx = __limitmeter_ctx;
...
pub fn patch_ls_wrapper(ctx: &rquickjs::Ctx<'_>) -> rquickjs::Result<()> {
    ctx.eval::<(), _>(
        r#"
        (function() {
            var rawFn = __limitmeter_ctx.host.ls._discoverRaw;
            __limitmeter_ctx.host.ls.discover = function(opts) {
                var optsJson;
...
pub fn patch_ccusage_wrapper(ctx: &rquickjs::Ctx<'_>) -> rquickjs::Result<()> {
    ctx.eval::<(), _>(
        r#"
        (function() {
            var rawFn = __limitmeter_ctx.host.ccusage._queryRaw;
            __limitmeter_ctx.host.ccusage.query = function(opts) {
...
mod tests {
    ...
    fn crypto_api_exposes_decrypt() {
        ...
        ctx.with(|ctx| {
            ...
            let globals = ctx.globals();
            let probe_ctx: Object = globals.get("__limitmeter_ctx").expect("probe ctx");
    ...
    fn crypto_api_decrypts_node_generated_envelope_from_js() {
        ...
        ctx.with(|ctx| {
            ...
            inject_host_api(&ctx, "test", &app_data, "0.0.0").expect("inject host api");
            let js_expr = format!(
                r#"__limitmeter_ctx.host.crypto.decryptAes256Gcm("{}", "{}")"#,
                envelope, key_b64
            );
    ...
    fn env_api_respects_allowlist_in_host_and_js() {
        ...
        ctx.with(|ctx| {
            ...
            for name in WHITELISTED_ENV_VARS {
                ...
                let js_expr = format!(r#"__limitmeter_ctx.host.env.get("{}")"#, name);
            ...
            let js_blocked: Option<String> = ctx
                .eval(r#"__limitmeter_ctx.host.env.get("__OPENUSAGE_TEST_NOT_WHITELISTED__")"#)
    ...
    fn env_api_prefers_process_env() {
        ...
        ctx.with(|ctx| {
            ...
            let js_value: Option<String> = ctx
                .eval(r#"__limitmeter_ctx.host.env.get("ZAI_API_KEY")"#)
                .expect("js get");
            assert_eq!(
                js_value.as_deref(),
                Some("sk-process-env-test-1234567890"),
    ...
    fn current_macos_keychain_account_prefers_explicit_user_value() {
        assert_eq!(
            current_macos_keychain_account_from_user_env(Some("limitmeter-test-user".to_string())),
            "limitmeter-test-user"
        );
    }
    ...
    fn ccusage_path_entries_with_home_and_existing_path_preserves_order() {
        let home = std::path::PathBuf::from("/tmp/limitmeter-home");
