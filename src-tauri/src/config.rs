...
/// Proxy configuration loaded from ~/.limitmeter/config.json
...
/// Config file path: ~/.limitmeter/config.json
fn config_path() -> Option<PathBuf> {
    dirs::home_dir().map(|home| home.join(".limitmeter").join("config.json"))
}
