...
const loadPlugin = async () => {
  ...
  return globalThis.__limitmeter_plugin;
};
...
function setKeychainToken(ctx, token) {
  ctx.host.keychain.readGenericPassword.mockImplementation((service) => {
    if (service === "LimitMeter-copilot") return JSON.stringify({ token });
    return null;
  });
}
...
describe("copilot plugin", () => {
  beforeEach(() => {
    delete globalThis.__limitmeter_plugin;
  ...
  it("loads token from LimitMeter keychain", async () => {
    const ctx = makePluginTestContext();
  ...
  it("does not persist token loaded from LimitMeter keychain", async () => {
    const ctx = makePluginTestContext();
  ...
  it("retries with gh-cli token when cached keychain token is stale", async () => {
    ...
    expect(ctx.host.keychain.deleteGenericPassword).toHaveBeenCalledWith("LimitMeter-copilot");
  ...
  it("falls back when LimitMeter keychain payload lacks token field", async () => {
    const ctx = makePluginTestContext();
    ctx.host.keychain.readGenericPassword.mockImplementation((service) => {
      if (service === "LimitMeter-copilot") return JSON.stringify({ notToken: "x" });
