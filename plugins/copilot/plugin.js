(function () {
  const KEYCHAIN_SERVICE = "LimitMeter-copilot";
    ...
    try {
      ...
      if (raw) {
        ...
        if (parsed && parsed.token) {
          ctx.host.log.info("token loaded from LimitMeter keychain");
          return { token: parsed.token, source: "keychain" };
        }
      }
    } catch (e) {
      ctx.host.log.info("LimitMeter keychain read failed: " + String(e));
    }
    return null;
  }
  ...
  function probe(ctx) {
    ...
    // Persist gh-cli token to LimitMeter keychain for future use
    if (source === "gh-cli") {
      saveToken(ctx, token);
    }
  ...
  globalThis.__limitmeter_plugin = { id: "copilot", probe };
})();
