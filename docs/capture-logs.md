...
Use this when LimitMeter is not working and you need to share debug info.
...
1. In your macOS menu bar, find the LimitMeter icon.
2. Right-click it (or hold `Control` and click).
3. Open `Debug Level`.
4. Select `Debug`.

If LimitMeter does not open at all, skip this step and continue.
...
```text
~/Library/Logs/com.kshitijkoranne.limitmeter
```
...
1. Attach `limitmeter.log`.
2. If you also see files like `limitmeter.log.1`, attach those too.
