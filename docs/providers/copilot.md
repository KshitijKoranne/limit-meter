...
1. **LimitMeter Keychain** (`LimitMeter-copilot`) — Token previously cached by the plugin
...
Once authenticated via gh CLI, the plugin caches the token in the LimitMeter keychain for faster access on subsequent probes.
