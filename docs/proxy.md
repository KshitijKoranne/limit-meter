...
LimitMeter can route provider and plugin HTTP requests through an optional proxy.

- Supported proxy types: `socks5://`, `http://`, `https://`
- Config file: `~/.limitmeter/config.json`
- Default: off
- UI: none

## Config File

Create `~/.limitmeter/config.json`:
...
- Restart LimitMeter after changing the file.
...
This applies to provider and plugin HTTP requests that go through LimitMeter's built-in HTTP client.
