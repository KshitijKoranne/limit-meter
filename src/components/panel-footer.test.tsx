...
describe("PanelFooter", () => {
  ...
  it("shows retryable updates soon state for update check failures", async () => {
    ...
    const retryButton = screen.getByRole("button", { name: "Update check unavailable" })
  ...
  it("shows error state for non-check failures", () => {
    ...
    expect(screen.queryByRole("button", { name: "Update check unavailable" })).toBeNull()
  })
  ...
  it("opens About dialog when clicking version in idle state", async () => {
    ...
    render(<Harness />)
    await userEvent.click(screen.getByRole("button", { name: /LimitMeter/ }))
